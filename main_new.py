...
class Reserver:
    def __init__(self,cfgs):
        ...
        # 请输入场地编号(玉泉羽毛球场(39)):
        self.venue_site_id = str(cfgs['place_id'])

        # 请输入预约日期(yyyy-mm-dd):
        self.date = cfgs['date']

        # 请输入候选开始时间(hh:mm)，空格隔开:
        self.candidate = [self.date + " " + i for i in cfgs['candidate_time']]
        # 请输入预约场数(1/2):
        self.n_site =str(cfgs['n_site'])

        # 请输入同伴姓名（必须一起预约过）:
        # companion_str = f.readline().replace('\n', '')
        self.companion = [id['buddyname'][0] for id in cfgs['configs']]
        # 请输入手机号:
...
class User(object):
    ...
    def order(self, buddy_no, reserver):
        ...
        for buddy in sorted(buddy_list, key=lambda i: i['id']):
            if self.config['buddyname'].count(buddy["name"])!=0:
...
def job(user, buddies, reserver,config, mode):
    buddy_no = ""
    for buddy in buddies:
        tmp = User(buddy['username'], buddy['password'], config)
        if buddy_no != "":
            buddy_no += ","
...
if __name__ == "__main__":
    ...
    configs = json.load(open("./config_new.json", encoding="utf-8"))
    main(configs)
